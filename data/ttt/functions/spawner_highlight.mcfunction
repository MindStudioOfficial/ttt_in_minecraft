...
execute as @e[tag=exp_barrel_spawner,tag=spawner] at @s if entity @a[predicate=ttt:admin_holding_spawner,distance=..50] run particle minecraft:smoke ~ ~.5 ~ 0.1 0 0.1 0 10 force

execute as @e[tag=minecart_spawner,tag=spawner] at @s if entity @a[predicate=ttt:admin_holding_spawner,distance=..50] run particle minecraft:angry_villager ~ ~.5 ~ 0.1 0 0.1 0 10 force
