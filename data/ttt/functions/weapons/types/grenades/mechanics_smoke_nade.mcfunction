...
execute at @e[tag=primed_smoke,scores={grenadetime=40}] run playsound custom.sg_explode block @a ~ ~ ~ 1 1

#* extiguish fire nade
execute at @e[tag=primed_smoke,scores={grenadetime=40}] if entity @e[tag=primed_fire,distance=..3] run playsound minecraft:block.fire.extinguish block @a ~ ~ ~ 1 1
execute at @e[tag=primed_smoke,scores={grenadetime=40}] if entity @e[tag=primed_fire,distance=..3] run kill @e[tag=primed_fire,distance=..3]

#* smoke particle
execute at @e[tag=primed_smoke,scores={grenadetime=40..}] run particle minecraft:squid_ink ~ ~1.9 ~-.5 1.5 1.5 1.5 0 500 force
