...
execute if entity @s[scores={c_ammo_rifle=1..}] at @e[tag=ray_rifle,limit=1,sort=nearest] as @e[tag=ray_rifle,limit=1,sort=nearest] if block ~ ~ ~ air run function ttt:raycast/tp
#* fire sound
execute if entity @s[scores={c_ammo_rifle=1..}] run playsound custom.scout_fire-1 player @a ~ ~ ~ 1 1
